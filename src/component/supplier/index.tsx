  ...
  return (
    ...
    >
      <Box display={"flex"} justifyContent={"space-between"}>
        <Typography>{keyword && `Showing ${supplierData?.metadata.total_data} suppliers for “${keyword}”`}</Typography>
      ...
      <Box sx={{ display: "flex" }}>
        ...
        <Box sx={{ paddingLeft: 3, width: "100%" }}>
          {isLoading || !supplierData ? (
            Array.from(Array(3)).map((_, id) => <SupplierItemSkeleton key={id} />)
          ) : (
            <>
              <Box width={"100%"} marginLeft={0} mt={0}>
                {supplierData?.companies.length > 0 ? supplierData?.companies?.map((supplier: SupplierDto) => (
                  <Box
                    key={supplier?.id}
                    width={"100%"}
                    height={"100%"}
                  ...
                  >
                    ...
                    <Box display={"flex"} justifyContent={"flex-end"} height={"fit-content"}>
                      ...
                      <Button
                        ...
                        onClick={() => router.push(`${SUPPLIER_PATH_URL.SUPPLIER_DETAIL}/${supplier.slug}`)}
                      >
                        View details
                      </Button>
                    </Box>
